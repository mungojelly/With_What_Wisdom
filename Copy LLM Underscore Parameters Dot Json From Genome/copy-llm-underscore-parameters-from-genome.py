...
def print_copy(subdir):
    print(f"📋🤖🔁📚💼 Copied llm_parameters.json in {subdir}")
...
def main():
    ...
    for subdir in os.listdir(inbox_path):
        ...
        if os.path.isdir(subdir_path):
            try:
                genome_file_path = os.path.join(subdir_path, 'genome', 'llm_parameters.json')
                destination_file_path = os.path.join(subdir_path, 'llm_parameters.json')
